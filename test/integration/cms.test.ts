...
const cmsTestSystem = defineSystem({
  id: 'cms-integration-test',
...
test(
  ...
  async () => {
    const app = await buildApp(
      ...
      { system: cmsTestSystem },
    );
    const testRunId = randomUUID().slice(0, 8);
    const email = `cms-owner-${testRunId}@example.test`;
    const password = 'Cms-owner-password-2026!';
    const slug = `frame-cms-test-${testRunId}`;
    ...
    const hidden = await app.inject({
      method: 'GET',
      url: `/api/public/cms/articles/${slug}`,
    });
    ...
    assert.ok(sitemap.body.includes(`/articles/${slug}`));
    ...
    assert.equal(redirected.headers.location, `/articles/${slug}`);

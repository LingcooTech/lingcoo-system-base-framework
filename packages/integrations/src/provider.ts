...
export function validateProviderFields(
  ...
  for (const field of fields) {
    ...
    if (field.required && !hasValue(fieldValue)) {
      throw integrationError(422, `${field.label}不能为空`, 'ValidationError');
    }
    ...
    if (!valid) throw integrationError(422, `${field.label}格式无效`, 'ValidationError');
    if (field.type === 'url') {
      try {
        const parsed = new URL(String(fieldValue));
        if (!['http:', 'https:'].includes(parsed.protocol)) throw new Error();
      } catch {
        throw integrationError(422, `${field.label}必须是 HTTP(S) 地址`, 'ValidationError');
      }
    }
  }
}
...
export class IntegrationProviderRegistry {
  ...
  register(provider: IntegrationProvider): void {
    ...
    this.manifests.set(provider.code, {
      code: provider.code,
      name: provider.name,
  ...
  requireAdapter(code: string): IntegrationProvider {
    ...
    if (!manifest) throw integrationError(422, '未知的集成 Provider', 'ValidationError');
    const adapter = this.adapters.get(code);
    if (!adapter) {
      throw integrationError(409, `${manifest.name}适配器尚未安装`, 'ConflictError');
    }
    return adapter;
  }
}

...
import { integrationConnections, integrationEvents } from '@lingcootech/frame-database/schema';
import { decryptSetting, encryptSetting, SettingsCryptoError } from './crypto.js';
import { integrationError } from './errors.js';
import type { IntegrationsPorts } from './ports.js';
import { createNoopIntegrationsPorts } from './ports.js';
...
function requireEncryptionKey(secret: string | undefined): string {
  if (!secret) throw integrationError(503, '集成凭据加密密钥尚未配置', 'ConfigurationError');
  return secret;
}
...
function decryptCredentials(connection: ConnectionRow, secret: string): Record<string, unknown> {
  try {
    ...
    if (error instanceof SettingsCryptoError) {
      throw integrationError(503, '集成凭据无法解密，请检查加密密钥', 'ConfigurationError');
    }
    throw error;
  }
}
...
export class IntegrationService {
  constructor(
    ...
    private readonly ports: IntegrationsPorts = createNoopIntegrationsPorts(),
  ) {}
  ...
  async findConnection(connectionId: string): Promise<ConnectionRow> {
    const [connection] = await this.db
      ...
      .limit(1);
    if (!connection) throw integrationError(404, '集成连接不存在', 'NotFoundError');
    return connection;
  }
  ...
  async createConnection(
    ...
    const [connection] = await this.db
      ...
      .values({
        ...
        name: input.name,
        config: input.config,
        encryptedCredentials: encryptSetting(
          input.credentials,
  ...
  async updateConnection(
    ...
    if (input.credentials) {
      ...
      const nextCredentials = { ...decryptCredentials(current, secret) };
    ...
    if (input.enabled === true && (invalidatesTest || current.lastTestStatus !== 'success')) {
      throw integrationError(409, '启用连接前必须使用当前配置完成连通性测试', 'ConflictError');
    }
  ...
  async executeConnection<T>(input: {
    ...
    if (connection.providerCode !== input.providerCode) {
      throw integrationError(422, '集成连接类型不匹配', 'ValidationError');
    }
    if (!connection.enabled) throw integrationError(409, '集成连接尚未启用', 'ConflictError');
    const provider = this.registry.requireAdapter(connection.providerCode);
    const credentials = decryptCredentials(connection, requireEncryptionKey(this.encryptionKey));
    validateProviderFields(provider.configFields, connection.config, '连接配置');
    validateProviderFields(provider.credentialFields, credentials, '访问凭据');
    ...
    try {
      ...
      await this.recordOperation(
        ...
        'failure',
        durationMs,
        redactMessage(error, credentials),
        input.actorId,
      );
      throw integrationError(502, '外部服务调用失败，请查看连接事件', 'IntegrationError');
    }
  }

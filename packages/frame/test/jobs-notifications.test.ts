...
import { computeBackoffMs, JobService } from '../src/core/modules/jobs/service.js';
...
test(
  ...
  async () => {
    ...
    const enabledSmtpConnections = await app.db
    ...
    for (const connection of enabledSmtpConnections) {
      await app.db

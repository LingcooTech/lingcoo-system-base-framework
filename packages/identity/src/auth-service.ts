...
export class AuthService {
  ...
  async login(
    ...
    if (!account) {
      ...
      throw identityError(401, '邮箱或密码不正确', 'UnauthorizedError');
    }
    ...
    if (account.status !== 'active') {
      throw identityError(403, '账号已停用', 'ForbiddenError');
    }
  ...
  async getPublicAccount(accountId: string) {
    ...
    if (!account) throw identityError(401, '账号不存在', 'UnauthorizedError');
    const access = await this.repository.getAccess(accountId);
    const avatarUrl = account.avatarAssetId
      ? ((await this.ports.avatars.resolvePublicImage(account.avatarAssetId))?.publicUrl ?? null)
      : null;
    return {
      id: account.id,
      email: account.email,
  ...
  async changePassword(
    ...
    if (!credential || !(await verifyPassword(input.currentPassword, credential.passwordHash))) {
      throw identityError(422, '当前密码不正确', 'ValidationError');
    }

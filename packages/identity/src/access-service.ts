...
import { normalizeRoleCode } from './rbac.js';
...
export class AccessService {
  ...
  constructor(
    ...
    this.security = new AccountSecurityService(db, ports);
  }
  ...
  async createAccount(
    ...
    if (await this.repository.findAccountByEmail(email)) {
      throw identityError(409, '该邮箱已被使用', 'ConflictError');
    }
    ...
    if (assignedRoles.length !== roleCodes.length) {
      throw identityError(422, '包含不存在的角色', 'ValidationError');
    }
  ...
  async resendInvitation(accountId: string, actorId: string) {
    ...
    if (account.emailVerifiedAt)
      throw identityError(409, '该账号邮箱已经验证，无需重新邀请', 'ConflictError');
  ...
  async updateAccount(
    ...
    if (accountId === actorId && input.status === 'suspended') {
      throw identityError(409, '不能停用当前登录账号', 'ConflictError');
    }
    ...
    if (removesOwner && (await this.repository.countOtherActiveOwners(accountId)) === 0) {
      throw identityError(409, '系统必须保留至少一个启用的所有者账号', 'ConflictError');
    }
  ...
  async createRole(
    ...
    const [existingRole] = await this.db
      ...
      .limit(1);
    if (existingRole) {
      throw identityError(409, '角色代码已存在', 'ConflictError');
    }
    ...
    if (knownPermissions.length !== permissionCodes.length) {
      throw identityError(422, '包含不存在的权限', 'ValidationError');
    }
  ...
  async updateRole(
    ...
    if (!role) throw identityError(404, '角色不存在', 'NotFoundError');
    if (role.isSystem) {
      throw identityError(409, '系统内置角色不能修改', 'ConflictError');
    }

...
function parseSettings(
  ...
  if (!/^[^\s/:]+$/.test(settings.host)) {
    throw integrationError(422, 'SMTP 主机格式无效', 'ValidationError');
  }
  if (!Number.isInteger(settings.port) || settings.port < 1 || settings.port > 65_535) {
    throw integrationError(422, 'SMTP 端口必须在 1 到 65535 之间', 'ValidationError');
  }
  if ([settings.user, settings.from].some((value) => /[\r\n]/.test(value))) {
    throw integrationError(422, 'SMTP 发件信息格式无效', 'ValidationError');
  }
  return settings;
}

...
function parseSettings(
  ...
  if (settings.keyType !== 'PKCS1' && settings.keyType !== 'PKCS8') {
    throw integrationError(422, '支付宝私钥格式只能是 PKCS1 或 PKCS8', 'ValidationError');
  }
  if (!/^\d{16,32}$/.test(settings.appId)) {
    throw integrationError(422, '支付宝应用 ID 格式无效', 'ValidationError');
  }
  ...
  if (
    ...
    throw integrationError(422, '支付宝网关必须使用官方 HTTPS 地址', 'ValidationError');
  }
  for (const [label, value] of [
    ...
    if (value && new URL(value).protocol !== 'https:') {
      throw integrationError(422, `${label}必须使用 HTTPS`, 'ValidationError');
    }
  }
  try {
    ...
    throw integrationError(422, '支付宝应用私钥或支付宝公钥不是有效 PEM', 'ValidationError');
  }

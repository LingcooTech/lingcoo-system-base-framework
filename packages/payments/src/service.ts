import { integrationError, type IntegrationService } from '@lingcootech/frame-integrations';
import { AlipayProvider } from './alipay.js';
import type { PaymentOrderInput, PaymentRefundInput } from './contracts.js';
        ...
        const value =
          provider instanceof AlipayProvider
            ...
            : provider instanceof WechatPayProvider
              ...
              : (() => {
                  throw integrationError(500, '支付 Provider 注册无效', 'ConfigurationError');
                })();
        return {
          value,
    ...
    return this.integrations.executeConnection({
      ...
      execute: async ({ provider, config, credentials }) => {
        if (!(provider instanceof AlipayProvider))
          throw integrationError(500, '支付宝 Provider 注册无效', 'ConfigurationError');
    ...
    return this.integrations.executeConnection({
      ...
      execute: async ({ provider, config, credentials }) => {
        if (!(provider instanceof WechatPayProvider))
          throw integrationError(500, '微信支付 Provider 注册无效', 'ConfigurationError');

...
function parseSettings(
  ...
  if (!/^[A-Za-z0-9_-]{6,64}$/.test(settings.appId))
    throw integrationError(422, '微信支付 AppID 格式无效', 'ValidationError');
  if (!/^\d{6,32}$/.test(settings.mchId))
    throw integrationError(422, '微信支付商户号格式无效', 'ValidationError');
  if (!/^[A-Fa-f0-9]{16,64}$/.test(settings.merchantSerialNo))
    throw integrationError(422, '商户证书序列号格式无效', 'ValidationError');
  if (!/^(?:[A-Fa-f0-9]{16,64}|PUB_KEY_ID_[A-Za-z0-9_]+)$/.test(settings.platformSerialNo))
    throw integrationError(422, '微信支付平台证书序列号或公钥 ID 格式无效', 'ValidationError');
  const baseUrl = new URL(settings.apiBaseUrl);
  if (
    baseUrl.protocol !== 'https:' ||
    !['api.mch.weixin.qq.com', 'api2.wechatpay.cn'].includes(baseUrl.hostname)
  )
    throw integrationError(422, '微信支付 API 地址必须使用官方 HTTPS 域名', 'ValidationError');
  if (new URL(settings.notifyUrl).protocol !== 'https:')
    throw integrationError(422, '微信支付通知地址必须使用 HTTPS', 'ValidationError');
  if (Buffer.byteLength(settings.apiV3Key) !== 32)
    throw integrationError(422, '微信支付 API v3 密钥必须为 32 字节', 'ValidationError');
  try {
    createPrivateKey(settings.merchantPrivateKey);
    createPublicKey(settings.platformPublicKey);
  } catch {
    throw integrationError(422, '微信支付商户私钥或平台公钥不是有效 PEM', 'ValidationError');
  }
  return settings;
}

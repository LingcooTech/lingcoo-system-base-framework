...
export class JobService {
  constructor(
    ...
    private readonly ports: JobsPorts = createNoopJobsPorts(),
  ) {}
  ...
  async retry(jobId: string, actorId: string) {
    ...
    if (!job) throw jobsError(404, '后台任务不存在', 'NotFoundError');
    if (!['dead', 'cancelled'].includes(job.status)) {
      throw jobsError(409, '只有死亡或已取消任务可以重新入队', 'ConflictError');
    }
    await this.db
      .update(jobRuns)
      .set({
  ...
  async cancel(jobId: string, actorId: string) {
    const [job] = await this.db.select().from(jobRuns).where(eq(jobRuns.id, jobId)).limit(1);
    if (!job) throw jobsError(404, '后台任务不存在', 'NotFoundError');
    if (job.status !== 'pending') throw jobsError(409, '只有等待中的任务可以取消', 'ConflictError');
    await this.db
      .update(jobRuns)

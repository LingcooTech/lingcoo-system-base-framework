...
export class CmsService {
  constructor(
    ...
    private readonly ports: CmsServicePorts,
  ) {}
  ...
  private async validateInput(input: CmsContentInput, currentId?: string) {
    ...
    if (assetIds.length) {
      if (!(await this.ports.assets.validatePublicImages(assetIds)))
        throw httpError(422, '内容图片必须是已启用的公开图片资产', 'ValidationError');
    }

    const termIds = [...new Set(input.termIds)];
    if (termIds.length) {
      if (!(await this.ports.taxonomy.validateActiveTerms(termIds)))
  ...
  private async enrich(row: typeof cmsContentEntries.$inferSelect) {
    const [author, terms, assets] = await Promise.all([
      row.authorId
        ...
        : null,
      this.ports.taxonomy.loadAssignedTerms(resourceType, row.id),
      [row.coverAssetId, row.socialImageAssetId].filter(Boolean).length
        ? this.ports.assets.loadPublicAssets(
  ...
  private async syncRelations(
    ...
    await this.ports.taxonomy.replaceAssignments(transaction, {
      resourceType,
      resourceId: contentId,
      termIds,
      actorId,
  ...
  async setStatus(contentId: string, status: 'draft' | 'published' | 'archived', actorId: string) {
    ...
    const [updated] = await this.db.transaction(async (transaction) => {
      const rows = await transaction
        ...
        .returning();
      if (status === 'published') {
        await this.ports.jobs.publishContent(transaction, {
          contentId,
          type: current.type,
          slug: current.slug,
  ...
  async schedule(contentId: string, publishAt: string | null, actorId: string) {
    ...
    const updated = await this.db.transaction(async (transaction) => {
      const [row] = await transaction
        ...
        .returning();
      if (scheduledAt) {
        await this.ports.jobs.schedulePublish(transaction, {
          contentId,

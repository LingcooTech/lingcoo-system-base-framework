CREATE TABLE IF NOT EXISTS "presentation_profiles" (
  ...
  "short_name" text,
  "slogan" text,
  "full_logo_asset_id" uuid,
  "square_logo_asset_id" uuid,
  "dark_logo_asset_id" uuid,
  "favicon_asset_id" uuid,
  "social_image_asset_id" uuid,
...
SELECT role.id, permission.code FROM "roles" role JOIN "permissions" permission
  ON permission.code IN ('presentation.read', 'presentation.write')
WHERE role.code IN ('owner', 'administrator') ON CONFLICT DO NOTHING;
INSERT INTO "role_permissions" ("role_id", "permission_code")
SELECT role.id, permission.code FROM "roles" role JOIN "permissions" permission
  ON permission.code = 'presentation.read'
WHERE role.code IN ('operator', 'viewer') ON CONFLICT DO NOTHING;

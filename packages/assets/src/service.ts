...
import { createNoopAssetsPorts, type AssetsPorts, type AssetStoragePort } from './ports.js';

type AssetRow = typeof storageAssets.$inferSelect;
type MediaKind = 'image' | 'video' | 'audio' | 'document' | 'archive' | 'other';

export type StorageAssetGateway = AssetStoragePort;
...
function safeDirectory(value: string | undefined): string {
  ...
  if (parts.some((part) => part === '..' || !/^[a-zA-Z0-9._-]+$/.test(part))) {
    throw assetsError(422, '上传目录格式无效', 'ValidationError');
  }
...
export class AssetService {
  constructor(
    ...
    private readonly ports: AssetsPorts = createNoopAssetsPorts(),
  ) {}
  ...
  private async requireAsset(assetId: string): Promise<AssetRow> {
    ...
    if (!asset) throw assetsError(404, '媒体资产不存在', 'NotFoundError');
    return asset;
  }

  private async resolveConnection(connectionId?: string) {
    const connection = await this.ports.storage.resolveConnection(connectionId);
    if (!connection) throw assetsError(422, '没有可用的对象存储连接', 'ValidationError');
    return connection;
  }
  ...
  async createUploadIntent(
    ...
    const [asset] = await this.db
      ...
      .returning();
    try {
      const token = await this.ports.storage.createUploadToken(
        connectionId,
        {
          key: objectKey,
          expiresInSeconds,
      ...
      const updated = await this.db.transaction(async (transaction) => {
        const [row] = await transaction
          ...
          .returning();
        await this.ports.jobs.enqueue(transaction, {
          queue: 'storage',
  ...
  async confirmUpload(assetId: string, actorId: string) {
    ...
    if (asset.status !== 'pending') {
      throw assetsError(409, '当前资产不能确认上传', 'ConflictError');
    }
    const object = await this.ports.storage.statObject(
      asset.connectionId,
      asset.objectKey,
      actorId,
    );
    ...
    if (expectedSize && object.size !== expectedSize) {
      await this.db
        ...
        .set({ status: 'failed', updatedAt: new Date() })
        .where(eq(storageAssets.id, asset.id));
      throw assetsError(422, '上传对象大小与上传意图不一致', 'ValidationError');
    }
  ...
  async getAccessUrl(assetId: string, actorId: string) {
    ...
    if (!['active', 'archived'].includes(asset.status)) {
      throw assetsError(409, '资产尚不可访问', 'ConflictError');
    }
    if (asset.visibility === 'public' && asset.publicUrl) {
      return { url: asset.publicUrl, expiresInSeconds: null };
    }
    const result = await this.ports.storage.createPrivateUrl(
      asset.connectionId,
  ...
  async update(
    ...
    const asset = await this.requireAsset(assetId);
    if (!['active', 'archived'].includes(asset.status)) {
      throw assetsError(409, '只有已启用或已归档资产可以修改', 'ConflictError');
    }
    const [updated] = await this.db
      .update(storageAssets)
      .set({
  ...
  async requestDelete(assetId: string, actorId: string) {
    const result = await this.db.transaction(async (transaction) => {
      ...
      if (!['active', 'archived', 'failed'].includes(asset.status)) {
        throw assetsError(409, '当前资产不能删除', 'ConflictError');
      }
      ...
      if (Number(references?.count ?? 0) > 0) {
        throw assetsError(409, '资产仍被业务资源引用，不能删除', 'ConflictError');
      }
      await transaction
        .update(storageAssets)
  ...
  async executeDelete(assetId: string) {
    ...
    await this.ports.storage.deleteObject(asset.connectionId, asset.objectKey);
    await this.db
      .update(storageAssets)
  ...
  async linkReference(
    ...
    return this.db.transaction(async (transaction) => {
      ...
      if (asset.status !== 'active') {
        throw assetsError(409, '只有已启用资产可以被引用', 'ConflictError');
      }
  ...
  async unlinkReference(assetId: string, referenceId: string, actorId: string) {
    const [removed] = await this.db
      ...
      .returning();
    if (!removed) throw assetsError(404, '资产引用不存在', 'NotFoundError');
    await this.ports.audit.record({

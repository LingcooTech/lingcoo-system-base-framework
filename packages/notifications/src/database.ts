...
export function resolveNotificationsDatabase(app: FastifyInstance): Database {
  ...
  if (!database) throw new Error('Notifications requires a configured PostgreSQL database adapter');
  return database;
}

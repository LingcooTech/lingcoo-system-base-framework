...
import { resolveNotificationsDatabase } from './database.js';
...
export function registerNotificationsRoutes(
  app: FastifyInstance,
  options: { ports?: NotificationsPorts } = {},
): void {
  const service = new NotificationService(
    resolveNotificationsDatabase(app),
    options.ports ?? createNoopNotificationsPorts(),
  );
